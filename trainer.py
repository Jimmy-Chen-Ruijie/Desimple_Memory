    ...
    def train(self, early_stop=False):
        ...
        for epoch in range(1, self.params.ne + 1):
            ...
            while not last_batch:
                ...
                heads, rels, tails, absolute_time, years, months, days = self.dataset.nextBatch(self.params.bsize, neg_ratio=self.params.neg_ratio)
                last_batch = self.dataset.wasLastBatch()
                
                scores = self.model(heads, rels, tails, absolute_time, years, months, days)
