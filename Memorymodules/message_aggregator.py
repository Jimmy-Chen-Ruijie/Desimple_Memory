...
class LastMessageAggregator(MessageAggregator):
  ...
  def aggregate(self, node_ids, messages):
    ...
    unique_node_ids = torch.unique(node_ids).cpu().numpy()
    unique_messages = []
    unique_timestamps = []
    ...
    for node_id in unique_node_ids:
        if len(messages[node_id]) > 0: #only use every node's message of last timestamp
    ...
    #index of node whose memory is to be updated
